...
SORT1       mov.w   #ARRAY1, R4             ; Initialize R4 to point to ARY1 in the memory
            mov.w   #ARRAY1S, R6            ; Initialize R6 to point to ARY1S where the sorted ARY1 will be stored
            ...
            call    #REPFR                  ; CALL the DUPLICATE Routine
...
;****************=>> Write Duplicate Entries Function <<=**********************/
;******************************************************************************/
...
LOOP        mov.w   #0x0000, SR             ; CLEAR ERRONEOUS SR bits
            ...
            mov.w   2(R4), R13              ; Backup next array index to R13
            cmp.w   @R4+, R13               ; compare LIST_IN[x] with LIST_IN[x + 1]
            jne     SKIP_COPY               ; DO NOT COPY to LIST_OUT if no duplicate

MATCH       cmp.w   -2(R4), -2(R5)          ; check if duplicate is already in the 3rd array
            jeq     SKIP_COPY               ; jump if the numbers are the same
  
COPY_X      mov.w   -2(R4), 0(R5)           ; Copy next element to LIST_OUT
            incd    R5                      ; iterate to LIST_OUT[m + 1]
            inc     R10                     ; increment 'm' in LIST_OUT[0]
SKIP_COPY   dec     R11                     ; decrement 'n' # LOOPS LEFT BEFORE EXIT
